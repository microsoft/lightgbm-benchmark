...
def analyze_inferencing(ws, experiment_id, benchmark_id, output_markdown_path):
    """
    """
    print("*** Fetching Experiment")
    experiment = Experiment(workspace=ws, name=experiment_id)

    print("*** Fetching Benchmark Runs")
    benchmark_runs = experiment.get_runs(tags={"benchmark_name" : benchmark_id, 'task': 'score'}, include_children=True)

    # iterate through runs to get all the data we need for the analysis
    print("*** Iterating through runs")

    benchmark_data = [] # list of dict for pandas
    for run in benchmark_runs:
        ...
        benchmark_data.append(benchmark_data_entry)
...
def run(args, unknown_args=[]):
    ...
    if args.template == 'inferencing':
        analyze_inferencing(ws, args.experiment_id, args.benchmark_id, args.output)
    else:
